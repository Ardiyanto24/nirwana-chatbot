# Katalog Data — Serving AI Chatbot

**Nirwana Hospitality Group — Data Platform ELT**

| | |
|---|---|
| **Dihasilkan oleh** | Dokumentasi tambahan (bukan milestone baru), disusun dari `scripts/chatbot_views/views_*.sql` (Milestone 4.2), `docs/01-architecture/Metadata.md`, dan `docs/07-mart-aggregated/DataSchema-mart-aggregated.md` |
| **Input utama** | `scripts/chatbot_views/views_*.sql` (definisi SQL 67 view), `docs/09-serving-ai-chatbot/pemetaan-akses-teknis-chatbot.md` (M4.1), `docs/09-serving-ai-chatbot/view-query-pattern-chatbot.md` (M4.2) |
| **Dipakai oleh** | Desain Lapis 1 (NL → query) di aplikasi AI Chatbot (di luar repo ini), developer `scripts/chatbot_api/` |
| **Status** | Selesai |
...
AI Chatbot **hanya pernah** mengakses satu permukaan data: schema `chatbot_views` di serving PostgreSQL (67 view, di-query lewat `GET /chatbot/{domain}/{view_name}` — lihat `docs/09-serving-ai-chatbot/api-chatbot.md`). Chatbot **tidak pernah** menyentuh `mart_aggregated`/`mart_cleaned` mentah secara langsung.

Dokumen ini adalah katalog isi permukaan data tersebut: **setiap view, kolom apa saja yang dimilikinya, dan pertanyaan bisnis apa yang bisa dijawabnya** — supaya siapa pun yang merancang logika "ubah pertanyaan bahasa natural jadi query" (Lapis 1, di aplikasi chatbot) tahu persis apa yang tersedia untuk dipakai, tanpa harus membaca SQL sumbernya satu per satu.

**Beda dari dokumen lain di folder ini:**
- `pemetaan-akses-teknis-chatbot.md` (M4.1) — RBAC 10 domain → tabel sumber teknis, level arsitektur, tidak mendaftar kolom.
- `view-query-pattern-chatbot.md` (M4.2) — catatan desain teknis "kenapa/bagaimana" tiap view dibangun, tanpa daftar kolom.
- `api-chatbot.md` (M4.4) — kontrak endpoint API, bukan katalog data.
- **Dokumen ini** — katalog isi: "apa saja yang ada di tiap view, kolomnya apa, dan untuk apa".

## Cara Membaca Dokumen Ini

Disusun per `data_domain` (10 domain, sesuai `corporate_master.role_permissions`), lalu per view di dalamnya (67 total). Tiap view punya:
- **Sumber** — tabel `mart_aggregated`/`mart_cleaned` asal + grain (level detail 1 baris).
- **Fungsi** — pertanyaan bisnis yang bisa dijawab view ini.
- **Kolom** — tabel nama kolom + makna bisnisnya.
- **Catatan** (bila ada) — business rule penting, kolom nullable/derived, atau batasan yang wajib diketahui sebelum query.

**Aturan lintas-domain yang berlaku di hampir semua view** — dijelaskan sekali di [§ Catatan Lintas-Domain](#catatan-lintas-domain) di akhir dokumen, tidak diulang di tiap view:
1. `property_id` di setiap view **selalu mentah, tidak difilter**. Pembatasan "hanya lihat properti sendiri" (`own_property`) adalah tanggung jawab API (M4.4), bukan view.
2. Tidak ada view domain operasional yang join ke `guests` — data tamu (nama/kontak/loyalitas) hanya lewat `guests_contact_view`/`guests_profile_view`.
3. Kode properti: `P01` Bali, `P02` Jakarta, `P03` Yogyakarta, `P04` Bandung, `P05` Lombok, `P06` Kantor Pusat (tidak punya kamar — hanya relevan untuk domain `financial`/`hr`/`employees_directory`).

---

## Ringkasan 10 Domain

| Domain | Isi | Sensitivitas | Pola akses properti | Jumlah view |
|---|---|---|---|---|
| `reservation` | Booking kamar, okupansi, revenue kamar, pricing | Rendah–Sedang | `own_property` (Front Office/Revenue Staff/Manager), `all_properties` (Corporate Revenue Director, GM, CEO) | 10 |
| `fnb` | Penjualan F&B, inventori bahan, resep, waste | Rendah | `own_property` (F&B Staff/Manager), `all_properties` (GM, CEO) | 11 |
| `facility` | Status kamar, housekeeping, tiket maintenance | Rendah (operasional), Sedang (data performa individu staff) | `own_property`; performa individu staff dibatasi ke diri sendiri untuk role Staff (di API) | 12 |
| `spa_event` | Booking spa & event/MICE, venue | Rendah | `own_property` (Spa & Event Staff/Manager), `all_properties` (GM, CEO) | 9 |
| `hr` | Kehadiran, performa karyawan, turnover — **tanpa payroll** | Sedang–Tinggi | `own_property` (HR Staff/Manager), `all_properties` (Corporate HR Director, CEO) | 10 |
| `financial` | Revenue/expense/profit/GOP, payroll — eksklusif domain ini | Tinggi | `own_property` (Finance Staff/Manager, GM), `all_properties` (Corporate Finance Director, CEO) | 11 |
| `properties_ref` | Master 6 properti (nama, region, tanggal buka) | Rendah | Umumnya `all_properties` — dipakai lintas persona untuk resolusi nama properti | 1 |
| `employees_directory` | Direktori karyawan (nama, properti, departemen, level akses) | Sedang–Tinggi (data pribadi karyawan) | Bergantung role — dipakai untuk resolusi nama karyawan | 1 |
| `guests_pii` | Kontak tamu: nama, email, telepon | Tinggi (PII) | `own_property` via `last_active_property_id` (properti aktivitas terakhir tamu) | 1 |
| `guests_profile` | Atribut analitis tamu: loyalitas, kebangsaan | Sedang | `own_property` via `last_active_property_id` | 1 |

**Total: 67 view** (50 sumber `mart_aggregated` — 48 agregat + 2 dimension-only, 17 sumber `mart_cleaned` row-level).

---

## Domain `reservation` (10 view)

Data booking kamar, okupansi, dan revenue kamar. `mart_aggregated` untuk tren/agregat, `mart_cleaned` untuk lookup real-time (status booking hari ini, ketersediaan kamar). **Dikecualikan**: `fact_revenue_pace_booking_snapshot` (pace booking, belum tersedia — status implementasi append-only belum final).
...
Penjualan F&B (restoran/bar/room service), inventori bahan baku, resep, dan waste. Rantai sebab-akibat: harga bahan naik → food cost naik → margin turun. **Tidak ada view basket analysis** (item yang sering dibeli bersamaan) — grain per struk hilang total di level agregat, dan tidak dimodelkan di row-level juga.
...
Status kamar, log housekeeping, dan tiket maintenance. Sensitivitas domain nominal "Rendah", **tapi data performa individu staff** (`v_housekeeping_staff_daily`, `v_maintenance_technician_daily`, dan kolom `staff_id`/`assigned_staff_id` di view lookup) lebih sensitif — role Staff hanya boleh lihat data miliknya sendiri, **ditegakkan di API (M4.4), bukan di view**.
...
Booking spa/wellness dan event/MICE, plus venue. **Tidak ada view repeat-client atau cross-sell spa×event** — `event_bookings` mengidentifikasi klien lewat `client_name` teks bebas (tanpa ID terstruktur), tidak ada kolom penghubung ke `spa_bookings`.
...
Kehadiran, performa, dan turnover karyawan. **Tidak ada view payroll sama sekali** di domain ini, baik agregat maupun row-level — payroll eksklusif domain `financial` (segregation of duties).
...
Revenue/expense/profit/GOP per departemen, dan payroll — **eksklusif domain ini**, tidak dapat diakses lewat domain `hr`. View paling berisiko double-counting ada di sini (lihat catatan `v_financial_departmental_margin`).
...
Domain paling sederhana — cuma untuk resolusi nama/metadata properti, dipakai lintas persona.
...
Direktori karyawan untuk resolusi nama/departemen/level akses. `full_name` diteruskan apa adanya (sudah diaudit aman untuk kebutuhan name-resolution eksplisit).
...
## Domain `guests_pii` / `guests_profile` (2 view di atas 1 tabel fisik)

Kedua view ini sama-sama membaca `mart_cleaned.guests`, tapi kolomnya dipisah berdasarkan sensitivitas — **desain sengaja**, bukan duplikasi. `guests` adalah **master pelanggan**, bukan hanya tamu yang menginap (berisi 18.000 tamu pernah menginap + 6.500 pelanggan lokal F&B/spa yang tidak pernah booking kamar).

Tabel `guests` **tidak punya kolom `property_id`** — kolom `last_active_property_id` di kedua view di bawah adalah turunan: properti tempat aktivitas terakhir tamu (booking kamar atau spa), diambil dari gabungan `bookings` + `spa_bookings`, diurutkan tanggal terbaru. **`event_bookings` tidak ikut dihitung** — tabel itu tidak punya kolom `guest_id` sama sekali (klien event diidentifikasi lewat `client_name` teks bebas).
...
- `docs/09-serving-ai-chatbot/pemetaan-akses-teknis-chatbot.md` — pemetaan RBAC 10 domain ke struktur teknis (M4.1), termasuk mekanisme filter `own_property`/`all_properties`.
- `docs/09-serving-ai-chatbot/view-query-pattern-chatbot.md` — catatan desain teknis tiap view (M4.2): kenapa dibangun begitu, koreksi yang ditemukan selama implementasi.
- `docs/09-serving-ai-chatbot/api-chatbot.md` — kontrak endpoint `GET /chatbot/{domain}/{view_name}` (M4.4).
- `docs/09-serving-ai-chatbot/kredensial-chatbot.md` — 10 kredensial Postgres read-only per domain (M4.3).
- `docs/01-architecture/Metadata.md` — data dictionary lengkap 23 tabel produksi (sumber makna kolom & pola data kotor/null bermakna).
- `docs/07-mart-aggregated/DataSchema-mart-aggregated.md` — struktur 27 dimensi + 49 fact table `mart_aggregated` (sumber sebagian besar view agregat di katalog ini).
- `scripts/chatbot_views/views_*.sql` — definisi SQL asli seluruh 67 view (sumber tunggal kebenaran; dokumen ini adalah rangkuman naratifnya).
