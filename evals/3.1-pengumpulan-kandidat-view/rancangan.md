...
> **KOREKSI (ditambahkan Checkpoint 9):** Anotasi "Baseline BM25 (lokal)" di tiap skenario B1-B5 di bawah ini SEMULA ditulis berdasarkan skrip eksplorasi dengan stopword filtering yang **belum diterapkan** ke `pencarian_bm25.py` yang sesungguhnya di-commit Checkpoint 5 (kesalahan proses: skrip eksplorasi dipakai untuk merancang skenario, tapi perbaikannya tidak ikut disalin ke kode produksi saat itu). Diverifikasi ulang terhadap `cari_bm25()` ASLI (tanpa stopword filtering): **kelimanya (B1-B5) sama-sama TIDAK memicu `perlu_fallback`** — bukan cuma 4/5 seperti draf awal dokumen ini. Checkpoint 9 menambahkan stopword filtering ke `pencarian_bm25.py` (root cause sebenarnya: kata fungsi umum seperti "yang"/"dan"/"di" muncul di hampir seluruh 67 teks korpus, membuat skor selalu positif untuk query apa pun). Angka final pasca-perbaikan ada di `audit.md` bagian "Koreksi Checkpoint 9". Anotasi di bawah ini DIPERTAHANKAN apa adanya (bukan diedit diam-diam) sebagai jejak rancangan asli — lihat `milestones/3.1-pengumpulan-kandidat-view/logs.md` Checkpoint 9 untuk kronologi lengkap kesalahan dan perbaikannya.
