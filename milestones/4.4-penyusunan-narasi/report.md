...
## Addendum (Revisit, 2026-08-18) — Konfirmasi Visual Jaeger

Follow-up wajib Bagian 5 (verifikasi visual Jaeger) selesai — Docker Desktop diaktifkan user, stack `infra/observability/` (`docker compose up -d`) dijalankan, dua skenario nyata dipanggil lewat `susun_narasi()` sungguhan dengan `setup_tracing()` produksi (`src/observability/tracing.py`) mengarah ke Collector lokal (`localhost:4317`).

**Skenario sukses** (campuran sumber, mirror KK1): `trace_id=c21e3cbba4a508f98f711435e601956d`, span `chat`, durasi 11.39 detik, seluruh 7 atribut kontrak terisi benar — `session.id="verifikasi-m44-jaeger"`, `turn.index=1`, `gen_ai.operation.name="chat"`, `gen_ai.request.model="qwen/qwen3-32b"`, `prompt.id="interpretation.narasi"`, `prompt.version=1`, `narrative.turn_reference=[2]` (paket bersumber `session_memory (turn 2)` terdeteksi benar), `gen_ai.usage.input_tokens=1257`, `gen_ai.usage.output_tokens=424`.

**Skenario gagal** (APIError disimulasikan): `trace_id=406d557d193716b66c896f1f5d85c111`, span `chat`, durasi 20ms (gagal cepat, sesuai desain — tanpa retry internal), `error.type="gagal_teknis"` TERISI benar pada span, `otel.status_code="ERROR"` (OTel SDK otomatis menandai span error karena exception keluar dari `with` block) — exception `APIError` dikonfirmasi tetap diteruskan apa adanya ke pemanggil (`decisions.md` Keputusan 5).

Kedua trace dikonfirmasi lewat query langsung ke Jaeger API (`GET localhost:16686/api/traces?service=milestone-4.4-verifikasi-jaeger`), mengikuti pola verifikasi `infra/observability/README.md` ("Cek langsung lewat Jaeger API, tanpa buka browser") — bukan simulasi/mock. **Bagian 5 di atas (butir "Verifikasi visual Jaeger TERTUNDA") dinyatakan SELESAI**, dipertahankan apa adanya di atas sebagai catatan sejarah (bukan dihapus), konsisten prinsip `CLAUDE.md` "jangan menghapus atau menyembunyikan sejarah error/perubahan arah."
